...
class Usager(db.Model):
    """Modèle pour les usagers de la bibliothèque"""
...
class Ouvrage(db.Model):
    ...
    def exemplaires_disponibles(self):
        ...
        empruntes = sum(1 for e in self.exemplaires 
                       if any(emp.statut == 'en_cours' for emp in e.emprunts))
...
class Exemplaire(db.Model):
    """Modèle pour les exemplaires individuels d'un ouvrage"""
    __tablename__ = 'exemplaires'
    
    id = db.Column(db.Integer, primary_key=True)
    ouvrage_id = db.Column(db.Integer, db.ForeignKey('ouvrages.id'), nullable=False)
    numero = db.Column(db.String(50), nullable=False)  # Numéro d'inventaire
    etat = db.Column(db.String(20), default='bon')  # bon, abîmé, perdu
    ...
    def est_disponible(self):
        """Vérifie si cet exemplaire est disponible"""
        emprunts_actifs = [e for e in self.emprunts if e.statut == 'en_cours']
...
class Emprunt(db.Model):
    ...
    statut = db.Column(db.String(20), default='en_cours')  # en_cours, retourné, en_retard
...
class Reservation(db.Model):
    ...
    statut = db.Column(db.String(20), default='active')  # active, annulée, honorée
...
class Bibliothecaire(db.Model, UserMixin):
    """Modèle pour les bibliothécaires (administrateurs)"""
